...
public class CustomScanner {
    ...
    private Language ls = new Language();
    ...
    public CustomScanner(String programFile, String PIFFile, String STFile) {
    ...
    public void buildPIF(List<Pair<String, Integer>> tokens) {
        ...
        for (Pair<String, Integer> tokenPair : tokens) {
            String character = tokenPair.getKey();

            if (ls.isOperator(character) || ls.isReservedWord(character) || ls.isSeparator(character)) {
                pif.add(character, new Pair<>(-1, -1));
            } else if (ls.isIdentifier(character)) {
                symbolTable.add(character);
                Pair<Integer, Integer> position = symbolTable.getPosition(character);
                pif.add("identifier", position);
            } else if (ls.isConstant(character)) {
                symbolTable.add(character);
                Pair<Integer, Integer> position = symbolTable.getPosition(character);
                pif.add("const", position);
            } else if (!invalidTokens.contains(character)) {
                invalidTokens.add(character);
                isLexicallyCorrect = false;
                System.out.println("Error at line " + tokenPair.getValue() + ": invalid token " + character);
            }
        }
